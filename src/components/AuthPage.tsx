...
interface AuthPageProps {
  ...
  onAuthSuccess?: (user: { name: string; email: string; avatar?: string; provider?: string }) => void;
}
...
export const AuthPage: React.FC<AuthPageProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (mode === 'signup') {
      ...
      if (!result.success) {
        setLoading(false);
        setAuthError({ message: result.error || 'Failed to create account.' });
        return;
      }
      ...
      if (!result.success) {
        setLoading(false);
        setAuthError({ message: result.error || 'Invalid email or password.' });
        return;
      }
    ...
    <div className="h-screen max-h-screen w-full bg-neutral-50 dark:bg-[#0a0a0c] text-neutral-900 dark:text-white relative overflow-hidden flex flex-col justify-between select-none">
      ...
      <header className="relative z-20 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-4 flex items-center justify-between shrink-0">
        ...
        <button
          ...
          className="flex items-center space-x-2 focus:outline-none opacity-95 hover:opacity-100 transition cursor-pointer group"
