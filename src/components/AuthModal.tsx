...
import { authenticateGoogleAccount } from '../lib/userStore';
      ...
      const timer = setTimeout(() => {
        if (googleBtnRef.current) {
          renderGoogleSignInButton(
            ...
            async (googleProfile) => {
              setAuthenticatedUser(googleProfile);
              setLoading(true);
              try {
                await authenticateGoogleAccount(googleProfile);
                setLoading(false);
                ...
                setLoading(false);
                setAuthError({ message: err?.message || 'Failed to sync Google account with database.' });
              }
            },
            (err) => {
