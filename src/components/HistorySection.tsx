...
export const HistorySection: React.FC<HistorySectionProps> = ({ user, onOpenAuth }) => {
  ...
  return (
    <section id="scan-history-section" className="w-full max-w-7xl mx-auto px-6 py-12">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        ...
        <div className="lg:col-span-6 space-y-6 z-10">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm sm:text-base text-neutral-600 dark:text-neutral-400 max-w-lg leading-relaxed pt-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
            <div className="p-4 rounded-2xl border border-neutral-200/70 dark:border-neutral-800 bg-neutral-50/60 dark:bg-neutral-900/60 flex items-start space-x-3.5">
              <div className="w-10 h-10 rounded-xl bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 flex items-center justify-center flex-shrink-0 shadow-xs">
                <Clock className="w-5 h-5 text-neutral-800 dark:text-neutral-200 stroke-[1.5]" />
              </div>
              <div>
          ...
          <div className="flex flex-wrap items-center gap-4 pt-3">
            {user ? (
              <button
                ...
                className="inline-flex items-center space-x-2.5 px-6 py-3.5 rounded-2xl bg-neutral-900 text-white dark:bg-white dark:text-neutral-900 font-semibold text-xs sm:text-sm hover:opacity-90 transition cursor-pointer shadow-md active:scale-95"
              >
              ...
              <button
                onClick={onOpenAuth}
                className="inline-flex items-center space-x-3 px-6 py-3.5 rounded-2xl bg-neutral-900 text-white dark:bg-white dark:text-neutral-900 font-semibold text-xs sm:text-sm hover:opacity-90 transition cursor-pointer shadow-md active:scale-95"
              >
                ...
                <span>Continue with Google to View History</span>
              </button>
            )}
            ...
            <div className="inline-flex items-center space-x-2 px-4 py-3 rounded-2xl border border-neutral-200/80 dark:border-neutral-800 bg-neutral-50/80 dark:bg-neutral-900/80 text-xs font-medium text-neutral-600 dark:text-neutral-400">
              <Lock className="w-3.5 h-3.5 text-neutral-500 stroke-[1.5]" />
        ...
        <div className="lg:col-span-6 flex justify-center lg:justify-end items-center relative select-none">
          <div className="relative w-80 sm:w-[420px] md:w-[480px] h-80 sm:h-[420px] md:h-[480px] flex items-center justify-center select-none">
        ...
        {showHistoryList && (
          <div className="mt-10 pt-8 border-t border-neutral-200 dark:border-neutral-800 space-y-6 animate-in fade-in slide-in-from-top-2 duration-200">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h3 className="text-xl font-extrabold text-neutral-900 dark:text-white">Scan Log & Records</h3>
              ...
              {historyItems.length > 0 && (
                <button
                  ...
                  className="inline-flex items-center space-x-1.5 px-3.5 py-2 rounded-xl border border-rose-200 dark:border-rose-900/60 bg-rose-50 dark:bg-rose-950/30 text-rose-700 dark:text-rose-400 text-xs font-semibold hover:bg-rose-100 dark:hover:bg-rose-900/50 transition cursor-pointer self-start sm:self-auto"
                >
            ...
            {filteredHistory.length === 0 ? (
              <div className="p-10 text-center border border-dashed border-neutral-200 dark:border-neutral-800 rounded-2xl space-y-2 bg-neutral-50/50 dark:bg-neutral-900/30">
                ...
                {filteredHistory.map((item) => {
                  ...
                  return (
                    <div
                      ...
                      className="p-4 rounded-2xl border border-neutral-200/80 dark:border-neutral-800 bg-neutral-50/60 dark:bg-neutral-900/60 hover:border-neutral-300 dark:hover:border-neutral-700 transition cursor-pointer space-y-3"
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3.5 truncate mr-3">
                          <div className="p-2.5 rounded-xl bg-white dark:bg-neutral-800 border border-neutral-200/80 dark:border-neutral-700 shrink-0">
                            {item.status === 'Safe' ? (
                              <SafeIcon className="w-5 h-5 text-emerald-500" />
                            ) : (
                              <ShieldAlert className={`w-5 h-5 ${item.status === 'Malicious' ? 'text-rose-500' : 'text-amber-500'}`} />
                            )}
                          </div>
                          <div className="truncate">
                            {isUrlTarget ? (
                              <a
                                ...
                                className="text-xs sm:text-sm font-bold text-blue-600 dark:text-blue-400 hover:underline inline-flex items-center space-x-1.5 truncate max-w-xs sm:max-w-xl group"
                              >
                                <span className="truncate">{item.target}</span>
                                <ExternalLink className="w-3.5 h-3.5 shrink-0 opacity-70 group-hover:opacity-100" />
                              </a>
                            ) : (
                              <h4 className="text-xs sm:text-sm font-bold text-neutral-900 dark:text-white truncate max-w-xs sm:max-w-xl">
                                {item.target}
                              </h4>
                            )}
                            <p className="text-[11px] text-neutral-400 flex items-center space-x-2 mt-0.5">
                              <span className="uppercase font-bold text-[10px] text-neutral-500">{item.targetType}</span>
                              <span>•</span>
