...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const handleRemoveHistoryItem = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    const updated = await removeScanFromHistory(id);
    setHistoryItems(updated);
  };

  const handleClearHistory = async () => {
    const updated = await clearScanHistory();
    setHistoryItems(updated);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4 bg-neutral-950/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div 
        className="w-full h-full sm:h-auto max-w-3xl bg-white dark:bg-[#1A1A1D] border-0 sm:border border-neutral-200/80 dark:border-neutral-800 rounded-none sm:rounded-3xl shadow-2xl overflow-hidden relative flex flex-col md:flex-row min-h-screen sm:min-h-[520px] transform animate-in zoom-in-95 duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 sm:top-5 sm:right-5 w-8 h-8 rounded-full bg-neutral-100 dark:bg-neutral-800 text-neutral-500 hover:text-neutral-900 dark:hover:text-white flex items-center justify-center transition cursor-pointer z-10"
        ...
        <div className="w-full md:w-64 bg-neutral-50/70 dark:bg-neutral-900/60 border-b md:border-b-0 md:border-r border-neutral-200/80 dark:border-neutral-800 p-4 sm:p-6 flex flex-col justify-between shrink-0">
          <div className="space-y-3 sm:space-y-6">
            <div className="pr-10 md:pr-0">
              <h2 className="text-lg sm:text-xl font-extrabold text-neutral-900 dark:text-white tracking-tight">
                {activeTab === 'profile' ? 'My Profile' : activeTab === 'database' ? 'Settings' : 'Account & Settings'}
              </h2>
              <p className="text-[11px] sm:text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                Manage profile, settings & scan history
              </p>
            </div>

            {/* Nav Tabs - Horizontal scrollbar on mobile, vertical stack on desktop */}
            <nav className="flex flex-row md:flex-col space-x-2 md:space-x-0 md:space-y-1.5 overflow-x-auto pb-1 md:pb-0 scrollbar-none">
              <button
                onClick={() => setActiveTab('profile')}
                className={`flex items-center space-x-2.5 px-3.5 py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition cursor-pointer shrink-0 ${
                  activeTab === 'profile'
          ...
          <div className="hidden md:block pt-4 border-t border-neutral-200/60 dark:border-neutral-800 text-[11px] text-neutral-400 space-y-1">
        ...
        {/* Right Content Area */}
        <div className="flex-1 p-5 sm:p-8 space-y-6 overflow-y-auto max-h-none sm:max-h-[520px]">
          {/* DATABASE TAB */}
          {activeTab === 'database' && (
          ...
          {activeTab === 'profile' && (
            <div className="space-y-6 animate-in fade-in duration-150">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-neutral-700 dark:text-neutral-300">Username</label>
                  <input
                    type="text"
                    readOnly
